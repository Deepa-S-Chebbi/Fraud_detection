...
if st.button("Predict Fraud"):
    ...
    st.header("Prediction Result : '{int(prediction)}'")
   

    if prediction == 1:
        st.error(f"Fraudulent Transaction Detected! (Probability: {prediction:.2f})")
    else:
        st.success(f"Transaction is Legitimate. (Probability: {1 - prediction:.2f})")